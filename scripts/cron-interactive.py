...
def get_task_ids():
    """Read task IDs from cron-tasks.json."""
...
class CursesMenu:
    def __init__(self, stdscr, title, items, subtitle=None):
        """
        items: list of (key, label) tuples, or list of strings
        """
        self.stdscr = stdscr
        self.title = title
        ...
        curses.init_pair(3, curses.COLOR_WHITE, curses.COLOR_BLUE)   # header
    ...
    def draw(self):
        ...
        max_visible = h - start_row - 3  # leave room for help line
        if max_visible < 1:
            max_visible = 1
...
class CursesPrompt:
    ...
    def run(self):
        ...
        while True:
            ...
            self.stdscr.addstr(h - 1, 0, prompt_str)
            self.stdscr.addstr(h - 1, len(prompt_str), self.value)
            self.stdscr.move(h - 1, len(prompt_str) + self.cursor_pos)
            self.stdscr.refresh()
...
def interactive_menu(stdscr):
    ...
    while True:
        ...
        # Run: needs task selection + confirm
        if action == "run":
            tasks = get_task_schedules()
            if not tasks:
                show_output(stdscr, action, "No tasks configured.")
                continue
            task_items = [(tid, f"schedule: {sched}") for tid, sched in tasks.items()]
            task_menu = CursesMenu(stdscr, "Select task to run now", task_items)
            task_choice = task_menu.run()
            if task_choice is None:
                continue
            output = run_cronctl("add", task_choice[0], "* * * * *", "manual run")
            # Actually just run it via openclaude directly
            show_output(stdscr, f"Running {task_choice[0]}...", output)
            continue
...
def main():
    # If arguments passed, dispatch directly to cronctl (non-interactive)
    if len(sys.argv) > 1:
